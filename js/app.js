...
function renderPhoenixList(items) {
  ...
  items.forEach((item, i) => {
    ...
    // ДОБАВЛЕНО: grid-column: 1 / -1; max-width: 500px;margin: 0 auto; 
    // Теперь анкета не будет сжиматься в 2 колонки, а займет всю ширину и будет выглядеть как красивый паспорт/досье.
    // Также высота фото (height: 300px) увеличена для гармонии с шириной.
    html += `
    <div class="shop-tile" style="grid-column: 1 / -1; max-width: 500px; margin: 0 auto; width: 100%; background: linear-gradient(180deg, rgba(255, 77, 77, 0.05) 0%, rgba(0,0,0,0.6) 100%); border: 1px solid rgba(255, 77, 77, 0.3); box-shadow: 0 8px 20px rgba(0,0,0,0.5); box-sizing: border-box;" onclick="toggleShop('${id}', this)">
      ...
      <div class="shop-tile-photo" style="height: 300px; padding: 0; background: transparent; border: none; border-radius: 10px; margin-bottom: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.5);">
        ${thumb}
      </div>
